    ...
    <div className="ch-screen ch-app" style={{ minHeight: '100vh' }}>
      <div className="ch-scroll" style={{ paddingBottom: 100 }}>
        <header className="ch-topbar">
          <Link to="/" style={{ background: 'none', border: 'none', cursor: 'pointer' }}><Icon name="arrowLeft" size={22} /></Link>
          <Link to="/settings" style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}><Icon name="settings" size={20} /></Link>
        </header>
        ...
        <div style={{ padding: '12px 24px 0', textAlign: 'center' }}>
          <div style={{
            ...
            fontFamily: 'var(--ch-serif)', fontSize: 36, color: 'var(--ch-ivory)',
          }}>{user.pseudo[0].toUpperCase()}</div>
          <div className="ch-serif" style={{ fontSize: 26, lineHeight: 1, margin: '0 0 4px' }}>{user.pseudo}</div>
          <div style={{ fontSize: 12, color: 'var(--ch-ink-mute)' }}>@{user.pseudo}{user.city ? ` · ${user.city}` : ''}</div>
        ...
        <div style={{ padding: '20px 20px 0', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
          {[
            ...
            { v: friendCount !== null ? String(friendCount) : '—', l: 'amis', to: '/social' },
          ].map((s, i) => {
            const inner = (
              <>
            ...
            return s.to ? (
              <Link key={i} to={s.to} className="ch-card" style={{ padding: 12, textAlign: 'center', textDecoration: 'none', color: 'inherit', display: 'block' }}>
                {inner}
              </Link>
            ) : (
              <div key={i} className="ch-card" style={{ padding: 12, textAlign: 'center' }}>
                {inner}
              </div>
            );
          })}
        ...
        {grids.length > 0 && (
          <div style={{ padding: '24px 20px 0' }}>
            <h3 className="ch-serif" style={{ fontSize: 22, margin: '0 0 12px', letterSpacing: '-0.01em' }}>Mes grilles</h3>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
              {grids.map(grid => (
                <div key={grid.id} className="ch-card" style={{ padding: 0, overflow: 'hidden' }}>
                  <img src={gridImgUrl(grid.imageUrl)} alt="Grille" style={{ width: '100%', display: 'block' }} />
                  <div style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10 }}>
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 500 }}>{grid.game?.inviteCode ?? '—'} · {grid.game?.mode === 'TEAM' ? 'Équipe' : 'Solo'}</div>
                      <div style={{ fontSize: 11, color: 'var(--ch-ink-mute)' }}>{new Date(grid.createdAt).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</div>
                    </div>
