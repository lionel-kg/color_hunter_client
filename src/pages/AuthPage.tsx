...
export function AuthPage() {
  ...
  return (
    <div className="ch-screen ch-app" style={{ minHeight: '100vh', padding: '0 24px', display: 'flex', flexDirection: 'column' }}>
      ...
      <h1 className="ch-serif" style={{ fontSize: 44, lineHeight: 1, margin: '20px 0 8px', letterSpacing: '-0.02em' }}>
        {mode === 'signup' ? <>Chasse la <em>couleur</em>.</> : <>Bon retour, <em>chasseur</em>.</>}
      </h1>
      ...
      <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
        {mode === 'signup' && (
          <input className="ch-input" placeholder="Pseudo" required value={pseudo} onChange={e => setPseudo(e.target.value)} />
        )}
        <input className="ch-input" type="email" placeholder="Email" required value={email} onChange={e => setEmail(e.target.value)} />
        <div style={{ position: 'relative' }}>
          <input className="ch-input" type={showPassword ? 'text' : 'password'} placeholder="Mot de passe" required value={password} onChange={e => setPassword(e.target.value)} minLength={mode === 'signup' ? 8 : 1} />
          <button type="button" onClick={() => setShowPassword(s => !s)} style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: 'var(--ch-ink-mute)', cursor: 'pointer', padding: 4 }}>
