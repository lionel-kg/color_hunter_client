...
export function GridCard({ grid, currentUserId, ownerActions, metaInfo }: Props) {
  ...
  useEffect(() => {
    api.get<{ liked: boolean; count: number }>(`/likes/${grid.id}`).then(r => {
  ...
  return (
    <article style={styles.card}>
        ...
        <div style={styles.header}>
          <div style={styles.avatar}>
            {grid.user.avatarUrl
              ...
              : <span style={styles.avatarInitial}>{grid.user.pseudo[0].toUpperCase()}</span>
            }
          </div>
      ...
      {/* Image grille */}
      <img src={grid.imageUrl} alt="Grille" style={styles.image} loading="lazy" />

      {/* Actions */}
      <div style={styles.actions}>
        <button onClick={toggleLike} style={{ ...styles.actionBtn, color: liked ? '#C97B7B' : 'var(--ch-ink-mute)' }}>
          <span style={{ fontSize: 18 }}>{liked ? '♥' : '♡'}</span>
          <span style={styles.count}>{likeCount}</span>
        </button>
        <button onClick={loadComments} style={{ ...styles.actionBtn, color: showComments ? 'var(--ch-ink)' : 'var(--ch-ink-mute)' }}>
      ...
      {showComments && (
        <div style={styles.commentsSection}>
          {comments.length === 0 && (
            <p style={styles.emptyComments}>Aucun commentaire pour l'instant.</p>
          )}
          {comments.map(c => (
            <div key={c.id} style={styles.comment}>
              ...
              {currentUserId === c.userId && (
                <button onClick={() => deleteComment(c.id)} style={styles.deleteBtn}>×</button>
              )}
            </div>
          ))}
          ...
          <form onSubmit={submitComment} style={styles.commentForm}>
            ...
            />
            <button type="submit" disabled={submitting || !commentText.trim()} style={styles.sendBtn}>
              ↑
            </button>
          </form>
        </div>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  avatarInitial: { fontFamily: 'var(--ch-serif)', fontSize: 14, color: 'var(--ch-ink-soft)' },
  pseudo: { fontFamily: 'var(--ch-sans)', fontSize: 13, fontWeight: 500, color: 'var(--ch-ink)' },
